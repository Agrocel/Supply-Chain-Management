...
logger = get_logger("Prophet_Tamil Nadu")

def train_prophet_model_tn(data):
    try:
        logger.info("Training Prophet Model for Tamil Nadu")
        prophet_tn_data = data[["Date", "QTY_MT"]].rename(columns = {"Date":"ds","QTY_MT":"y"}).copy()
    except Exception as e:
        logger.error(f"Error in Training Prophet Model for Tamil Nadu :{e}",exc_info=True)
        raise ValueError(f"Error in Training Prophet Model for Tamil Nadu")
    ...
    try:
        ...
        prophet_tn_data_pred = model.predict(prophet_tn_data)
    ...
    try:
        ...
        with open(os.path.join(Base, "..", "..", config['Prophet_model_TamilNadu']), 'w') as fout:
            fout.write(model_to_json(model))
        logger.info("Model Saved")

        logger.info("Creating Report for Prophet Tamil Nadu model")

        timestamp = pd.Timestamp.now().strftime("%Y-%m-%d_%H-%M-%S")
        save_model_error = pd.DataFrame({
            "Date" : [timestamp],
            "Model" : ['Prophet_Tamil Nadu'],
            "MAE" : [MAE],
            "RMSE" : [RMSE]
        })
        save_model_error.to_csv(config[f'model_evaluation_TamilNadu'], mode = 'a', index = False, header=not pd.io.common.file_exists(config[f'model_evaluation_TamilNadu']))
    ...
    return prophet_tn_data,forecast_future,prophet_tn_data_pred